...
export default function PropertiesPage() {
  ...
  const saveUnit = async (payload: UnitSavePayload, existingUnit: Unit | null) => {
    ...
    try {
      ...
      const relationRows = [
        owner && !owner.inherited && ownerContact && {
  ...
  return <AppLayout><div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[#f8f7f5]">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto"><div className="mx-auto max-w-[1240px] px-5 py-6 sm:px-8 sm:py-8">
      {feedback && <div data-testid={`feedback-properties-${feedback.tone}`} className={`mb-5 flex items-center justify-between rounded-[10px] border px-3 py-2.5 text-[12px] ${feedback.tone === 'success' ? 'border-[#d2ead7] bg-[#f2faf3] text-[#327443]' : 'border-[#f0c8c3] bg-[#fff7f6] text-[#a33a30]'}`}><span className="flex items-center gap-2">{feedback.tone === 'success' ? <Check size={14} /> : <AlertCircle size={14} />}{feedback.message}</span><button data-testid="button-dismiss-properties-feedback" onClick={() => setFeedback(null)}><X size={14} /></button></div>}
       <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-[13px] text-[#706c64]">A clear home for every place your team looks after.</p><p className="mt-1 text-[11px] text-[#a19c93]">{properties.length} {properties.length === 1 ? 'property' : 'properties'} in your portfolio</p></div><div className="flex w-full items-center gap-2 sm:w-auto"><label className="relative min-w-0 flex-1 sm:min-w-[230px]"><Search size={12} className="pointer-events-none absolute left-[10px] top-1/2 -translate-y-1/2 text-[#aaa59d]" /><input data-testid="input-property-search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name or address" className="h-[30px] w-full rounded-[8px] border border-[#e5e2dd] bg-[#faf9f7] pl-7 pr-7 text-[12px] text-[#151412] outline-none placeholder:text-[#b3b0aa] focus:border-[#cfcac2] focus:bg-white" />{search && <button type="button" onClick={() => setSearch('')} aria-label="Clear property search" className="absolute right-2 top-1/2 -translate-y-1/2 text-[#aaa59d] hover:text-[#151412]"><X size={11} /></button>}</label><PageFilterMenu testId="button-property-filters" activeCount={[typeFilter, statusFilter].filter(Boolean).length} onReset={() => { setTypeFilter(''); setStatusFilter(''); }}><FlowPointSelect testId="select-property-type-filter" value={typeFilter} onChange={setTypeFilter} placeholder="All types" options={PROPERTY_TYPES.map((type) => ({ value: type.value, label: type.label }))} /><FlowPointSelect testId="select-property-status-filter" value={statusFilter} onChange={setStatusFilter} placeholder="All statuses" options={PROPERTY_STATUSES.map((status) => ({ value: status.value, label: status.label }))} /></PageFilterMenu></div></div>
